...
public class SQLSourceHelperTest {
	...
	@Test
	public void incrementalQuery() {
		when(context.getString("incremental.column")).thenReturn("sysrq_");
		when(context.getString("custom.query")).thenReturn("select fphm_,czy_,zckhh_,je_,sysrq_ from cw_yhzz where " +
				"UNIX_TIMESTAMP(sysrq_) >= $@$ and UNIX_TIMESTAMP(sysrq_) < $#$ ORDER BY sysrq_ ASC");
		when(context.getString("status.file.path")).thenReturn("16778784");
		when(context.getString("status.file.name")).thenReturn("16778784");
		SQLSourceHelper sqlSourceHelper = new SQLSourceHelper(context,"Source Name");
		sqlSourceHelper.setCurrentIndex("16778784");
		System.out.println(sqlSourceHelper.buildQuery("1629907200"));
	}
	
	@Test
	public void getColumnsToSelect() {
		when(context.getString("columns.to.select")).thenReturn("sysrq_");
		SQLSourceHelper sqlSourceHelper = new SQLSourceHelper(context,"Source Name");
//		System.out.println(sqlSourceHelper.getContext().getString("columns.to.select"));
		System.out.println(sqlSourceHelper.getQuery());
	}
